...
const {toastSuccessMock,toastErrorMock} = vi.hoisted(()=>({toastSuccessMock:vi.fn(),toastErrorMock:vi.fn()}))
...
vi.mock("../../store/useSkillLevelStore",()=>({
    useSkillLevelStore: ()=>({
        ...
        levels:[{id:1,name:"Beginner"},{id:2,name:"Intermediate"}],
...
describe("AddSkillPage",()=>{
    ...
    const getElements = async () => {
         ...
         const skillLevelSelectElement = screen.getByRole('combobox') || screen.getByDisplayValue("Select Skill Level");
    ...
    it("submits the valid form and navigates",async ()=>{
        ...
        await waitFor(()=>{
            ...
            expect(addSkillMock).toHaveBeenCalledWith({ name: "React", skill_level_id: "1" });
